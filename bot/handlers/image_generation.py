...
async def generate_hu_tao(message: Message):
    ...
    if sh_token is None:
        return (
            "Вы не указали токен от stable horde. Токен можно получить после "
            "создания аккаунта тут: http://stablehorde.net/register\nУказать "
            "токен можно В ЛС ГРУППЫ с помощью команды !shapi <токен>"
        )
    sh_token = sh_token['sh_token']

    can_generate_photo = await message.ctx_api.messages.is_messages_from_group_allowed(
        group_id=GROUP_ID, user_id=message.from_id
    )
